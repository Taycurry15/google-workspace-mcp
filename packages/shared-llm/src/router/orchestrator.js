...
export class LLMOrchestrator {
    ...
    async generate(request) {
        ...
        for (const entry of ranked) {
            ...
            try {
                ...
                if (process.env.DEBUG_LLMS === "true") {
                    console.debug(`[LLM] ${provider.name} selected (score: ${entry.score.toFixed(2)})`);
                }
                return response;
            }
            catch (error) {
        ...
        throw new Error(`All LLM providers failed. Selection trail: ${reasons.join(" | ")}. Last error: ${lastError}`);
    }
    rankProviders(request) {
        const estimatedInputTokens = request.metadata?.estimatedInputTokens ?? this.estimateTokens(request.messages);
        ...
        const ranked = this.providers.map((provider) => {
            ...
            if (request.metadata?.allowLongContext ||
            ...
            // Budget awareness
            const estimatedCost = (estimatedInputTokens / 1_000_000) * provider.cost.inputPerMillion +
    ...
    taskHeuristic(providerName, taskType) {
        switch (taskType) {
            case "classification":
