 ...
 */
const DEFAULT_ROUTER_CONFIG: LLMRouterConfig = {
...
export class LLMRouter extends EventEmitter {
   ...
   */
  private filterCandidates(criteria: LLMSelectionCriteria): ModelCapabilities[] {
    let candidates = Object.values(MODEL_PRICING);

    // Filter by task type
    if (criteria.taskType) {
      candidates = candidates.filter((model) =>
        model.capabilities.includes(criteria.taskType!)
      );
    }
    ...
    if (criteria.contextSize) {
      candidates = candidates.filter(
        (model) => model.maxContextTokens >= criteria.contextSize!
      );
    }
    ...
    if (criteria.latencyRequirement) {
      ...
      candidates = candidates.filter(
        (model) => model.averageLatencyMs <= maxLatency
      );
    }
    ...
    for (const fallbackConfig of fallbackChain) {
      try {
        ...
        if (!error.retryable && fallbackChain.length > 1) {
          console.warn(
            `Non-retryable error from ${fallbackConfig.provider}/${fallbackConfig.model}, trying fallback`
          );
          continue;
        }
        ...
        if (error.retryable) {
          for (let i = 0; i < this.config.maxRetries; i++) {
            ...
            try {
              ...
              lastError = retryError as LLMError;
            }
          }
        }
      }
   ...
   * Merge config with defaults
   */
  private mergeConfig(config: Partial<LLMConfig>): LLMConfig {
    return {
   ...
   */
  private getBudgetThreshold(budget: "low" | "medium" | "high" | "unlimited"): number {
    switch (budget) {
      case "low":
   ...
   */
  private getLatencyThreshold(requirement: "low" | "medium" | "high"): number {
    switch (requirement) {
      case "low":
        return 500; // 500ms
      case "medium":
