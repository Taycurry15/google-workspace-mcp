...
import { Risk, RiskStatus } from "../types/pmo.js";
...
export async function readRisks(
  ...
  for (const row of rows) {
    ...
    const risk: Risk = {
      ...
      // PARA integration - critical risks need review
      para_needs_review: riskScore > 15,
    };
...
export async function createRisk(
  auth: OAuth2Client,
  spreadsheetId: string,
  risk: Omit<Risk, "id" | "score" | "para_needs_review">
): Promise<string> {
  const sheets = google.sheets({ version: "v4", auth });
...
export async function batchCreateRisks(
  auth: OAuth2Client,
  spreadsheetId: string,
  risks: Omit<Risk, "id" | "score" | "para_needs_review">[]
): Promise<string[]> {
  if (risks.length === 0) {
    return [];
  }
