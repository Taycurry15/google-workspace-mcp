...
export class AnthropicProvider {
    ...
    handleError(error, config) {
        ...
        return new LLMError(error.message || "Anthropic API error", "anthropic", config.model, error.error?.type || "unknown_error", error.status, retryable);
    }
}
/**
 * OpenAI Provider
...
export class OpenAIProvider {
    ...
    handleError(error, config) {
        const retryable = error.status === 429 || error.status >= 500;
        return new LLMError(error.message || "OpenAI API error", "openai", config.model, error.code || "unknown_error", error.status, retryable);
    }
}
/**
...
export class GoogleProvider {
    ...
    handleError(error, config) {
        return new LLMError(error.message || "Google API error", "google", config.model, "unknown_error", undefined, true);
    }
}
/**
 * Provider Factory
